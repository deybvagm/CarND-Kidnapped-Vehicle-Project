...
inline  double convertCarToMapCoordinate(double xCar, double yCar, double thetaMap, double axisValue, const std::string &axis){
    return (axis == "x") ? axisValue + (cos(thetaMap) * xCar) - (sin(thetaMap) * yCar) : axisValue + (sin(thetaMap) * xCar) + (cos(thetaMap) * yCar);
}
